...
public class AdminOrderPageController implements Initializable {
		...
		try {
			...
			ordertable.setOnMouseClicked((MouseEvent event) -> {
				if (event.getClickCount() == 1) {
					System.out.println("clicked order table");
	...
	private void setCellValueFromTableToTextField() {
	    if (ordertable.getSelectionModel().getSelectedItem() != null) {
	        ...
	        orderid.setText((selectedItems.toString().split(",")[0].substring(selectedItems.toString().split(",")[0].lastIndexOf("=") + 1)).replaceAll("\\s+", ""));
	        totalprice.setText((selectedItems.toString().split(",")[1].substring(selectedItems.toString().split(",")[1].lastIndexOf("=") + 1)).replaceAll("\\s+", ""));
	        totalpriceplustax.setText((selectedItems.toString().split(",")[2].substring(selectedItems.toString().split(",")[2].lastIndexOf("=") + 1)).replaceFirst("\\s+", ""));
	        orderdate.setText((selectedItems.toString().split(",")[3].substring(selectedItems.toString().split(",")[3].lastIndexOf("=") + 1)).replaceFirst("\\s+", ""));
	        orderstatus.setText((selectedItems.toString().split(",")[4].substring(selectedItems.toString().split(",")[4].lastIndexOf("=") + 1)).replaceFirst("\\s+", ""));
	    }
	}
